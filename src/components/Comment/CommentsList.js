...
export default function CommentsList() {
  ...
  const userAuth = useSelector((state) => state?.users?.userAuth?._id);

  const comments = useSelector((state) => state?.post?.details?.comment);
  ...
  return (
    <div>
      <ul className="divide-y bg-gray-700 w-96 divide-gray-200 p-3 mt-5">
        ...
        {comments?.length <= 0 ? (
          <h2 className="text-yellow-400 text-lg text-center">No comment</h2>
        ) : (
