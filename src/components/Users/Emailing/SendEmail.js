...
import { sendMailAction } from "../../../redux/slices/sendmail/sendEmailSlice";
...
const SendEmail = () => {
  const params = window.location.href.toString();
  ...
  const formik = useFormik({
    initialValues: {
      recipientEmail: profile?.email || query,
      subject: "",
      message: "",
    },
    onSubmit: (values) => {
  ...
  const { isMailSent, appErr, serverErr, loading } = useSelector(
    (state) => state?.mail
  );

  if (isMailSent) {
    return <Navigate to={profile ? `/profile/${profile?.id}` : `/users`} />;
  }
  return (
