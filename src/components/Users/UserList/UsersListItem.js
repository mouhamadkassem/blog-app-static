...
const UsersListItem = () => {
  ...
  const { loading, appErr, serverErr, usersList, block, unblock } = users;
  ...
  return (
    <>
      {loading ? (
        ...
        <h2 className="text-red-400 text-center text-3xl">
          {serverErr} {appErr}
        </h2>
      ) : usersList?.length <= 0 ? (
        <h2 className="text-yellow-400 text-center text-3xl ">NO USERS</h2>
      ) : (
        usersList?.map((user) => (
          <>
